...
def main():
	...
	parser.add_argument('--save-interval', default=1, type=int, help='save frequency in epochs (default: 10)')
	...
	if args.dataset == 'mnist':
	    ...
	    # mnist_train = np.load('D:/project/CMU/toytask/mnist_rotated/images_train.npy')
	...
	x_coord = torch.from_numpy(x_coord).float() #mesh grid of x and y coordinates each corresponding to one of the 784 pixel values

	# y_train = mnist_train.view(-1, n*m)
	y_test = mnist_test.view(-1, n*m)#image pixel values for 10k imges
	print(y_test.shape,x_coord.shape)# batch of actual images pixel values, artificial data of x and y coords
	## set the device
	d = args.device
	...
	elbo_accum,gen_loss_accum,kl_loss_accum = eval_model(test_iterator, x_coord, p_net,q_net, rotate=rotate, translate=translate,dx_scale=dx_scale, theta_prior=theta_prior,use_cuda=use_cuda)
